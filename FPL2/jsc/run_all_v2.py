...
def run(cfg: dict):
    ...
    if pruning_method in ('random', 'magnitude', 'random_init', 'synflow', 'snip', 'grasp', 'spectral_sensitivity'):
        from utils.pruning import (random_prune_to_ebops, magnitude_prune_to_ebops,
                                    random_init_prune_to_ebops, snip_prune_to_ebops,
                                    grasp_prune_to_ebops, synflow_prune_to_ebops)
        ...
        elif pruning_method == 'random_init':
            random_init_prune_to_ebops(model, warmup_ebops, _sample, verbose=True)
        elif pruning_method == 'synflow':
            synflow_prune_to_ebops(model, warmup_ebops, _sample, verbose=True)
        elif pruning_method == 'snip':
            _probe_n = min(2048, len(X_train))
            _probe_x = tf.constant(X_train[:_probe_n], dtype=tf.float32)
            _probe_y = tf.constant(y_train[:_probe_n], dtype=tf.int64)
            snip_prune_to_ebops(model, warmup_ebops, _probe_x, _probe_y, verbose=True)
        elif pruning_method == 'grasp':
            _probe_n = min(2048, len(X_train))
            _probe_x = tf.constant(X_train[:_probe_n], dtype=tf.float32)
            _probe_y = tf.constant(y_train[:_probe_n], dtype=tf.int64)
            grasp_prune_to_ebops(model, warmup_ebops, _probe_x, _probe_y, verbose=True)
        elif pruning_method == 'spectral_sensitivity':
            _, used_structured = spectral_sensitivity_prune_to_ebops(
                model,
                target_ebops=warmup_ebops,
                sample_input=_sample,
                min_degree=cfg.get('spectral_sens_min_degree', cfg['min_degree']),
                b_floor=cfg.get('spectral_sens_b_floor', 0.35),
                b_ceiling=cfg.get('spectral_sens_b_ceiling', 6.0),
                pruned_threshold=cfg.get('spectral_sens_pruned_threshold', 0.1),
                protect_power=cfg.get('spectral_sens_protect_power', 0.5),
                verbose=True,
            )
        if pruning_method != 'spectral_sensitivity':
    ...
    if pruning_method == 'spectral_sensitivity' and 'spectral_sens_allow_connection_kill' in cfg:
        allow_connection_kill = bool(cfg['spectral_sens_allow_connection_kill'])
    ...
    calibrated_ebops = bisect_ebops_to_target(
        ...
        allow_connection_kill=allow_connection_kill,
    )

#!/usr/bin/env python3
"""
run_experiment_G_restart_decay.py — restart_decay 精细消融 (multi-target eBOPs)
================================================================================
...
消融设计 (7 configs):
  ── 对照 ──
  G0: curriculum DISABLED (无停滞检测，纯 BetaOnlyBudgetController)
  G1: decay=0.00  (最激进: 每次重启 beta → beta_min)
  ...
  ── 上界 ──
  G6: decay=1.00  (重启不减 beta, 仅有 RECOVER 阶段的临时降压)

用法:
  python run_experiment_G_restart_decay.py --list
  python run_experiment_G_restart_decay.py --run G
  python run_experiment_G_restart_decay.py --run G0 G3
  python run_experiment_G_restart_decay.py --dry-run --run G
  python run_experiment_G_restart_decay.py --skip-existing --run G
...
def _target_output_root(target_ebops: int) -> str:
    return os.path.join(OUTPUT_ROOT, f'target_{target_ebops}')
...
EXPERIMENTS = {
    # ── G0: Curriculum disabled (真正的 baseline) ────────────────────────
    'G0': dict(
    ...
    # ── G1: decay=0.00 (最激进: beta → beta_min) ────────────────────────
    'G1': dict(
...
def build_config(exp_name: str, target_ebops: int) -> dict:
    ...
    # ★ Force spectral-quant pruning (not sensitivity-aware) for G ablation
    # G experiments focus on restart_decay tuning, not pruning method, so use spectral
    # cfg['use_sensitivity_pruner'] = False

    cfg['output_dir'] = os.path.join(_target_output_root(target_ebops), exp_name)
    return cfg
...
def run_single(exp_name: str, target_ebops: int, dry_run: bool = False) -> dict | None:
    exp_def = EXPERIMENTS[exp_name]
    cfg = build_config(exp_name, target_ebops)
    ...
    result['target_ebops'] = target_ebops
    return result
...
def _read_trace_best(output_dir: str, target_ebops: int):
...
def print_comparison(results: list[dict], target_ebops: int):
    import numpy as np
    output_root = _target_output_root(target_ebops)
    ...
    print(f'  {"Exp":>3s}  {"decay":>5s}  {"Description":<28s}  '
          ...
          f'{"#Rst":>4s}  {"Time":>5s}  {"Δ G0":>7s}  {"Δ G3":>7s}')
    print(f'  {"-" * 111}')

    # 找 G0 (disabled) 和 G3 (default) 的 acc
    ref = {}
    for r in results:
        exp = r.get('experiment', '')
        d = os.path.join(output_root, exp)
        acc, _ = _read_trace_best(d, target_ebops)
        if acc is None:
    ...
    for r in sorted(results, key=lambda x: x.get('experiment', '')):
        ...
        best, ep = _read_trace_best(d, target_ebops)
    ...
    if d0_acc is not None and d3_acc is not None:
        print(f'\n  G0 (no curriculum) = {d0_acc:.4f}  |  G3 (default, decay=0.25) = {d3_acc:.4f}  |  Δ = {d3_acc - d0_acc:+.4f}')
...
def main():
    parser = argparse.ArgumentParser(
        description='restart_decay ablation at multiple eBOP targets')
    parser.add_argument('--run', nargs='+', default=None,
                        help='Experiments to run (G0..G6, "G" or "all" for all)')
    parser.add_argument('--targets', nargs='+', type=int, default=TARGET_EBOPS_LIST,
                        help=f'eBOP targets to run (default: {TARGET_EBOPS_LIST})')
    ...
    if args.list:
        print(f'\n  restart_decay Ablation (targets={args.targets} eBOPs):')
    ...
    if args.compare_only:
        found_any = False
        for target_ebops in args.targets:
            results = []
            output_root = _target_output_root(target_ebops)
            for name in sorted(EXPERIMENTS.keys()):
                ...
                if os.path.isfile(rp):
                    ...
                    r['target_ebops'] = target_ebops
                    results.append(r)
            if results:
                print_comparison(results, target_ebops)
                found_any = True
        if not found_any:
    ...
    if args.run_all:
        ...
        parser.print_help()
        print(f'\n  --list to see configs, --run G to run all, --compare-only for results.')
        return

    print(f'\n{"#" * 72}')
    print(f'  restart_decay Ablation @ targets {args.targets} eBOPs')
    ...
    for target_ebops in args.targets:
        print(f'\n{"=" * 72}')
        print(f'  Target loop: {target_ebops} eBOPs')
        print(f'{"=" * 72}')
        ...
        for name in exp_names:
            ...
            if args.skip_existing and os.path.isfile(rp):
                print(f'\n  [Skip] {name}: result exists (target={target_ebops})')
                with open(rp) as f:
                    r = json.load(f)
                r['experiment'] = name
                r['desc'] = EXPERIMENTS[name]['desc']
            ...
            if args.dry_run:
                run_single(name, target_ebops, dry_run=True)
                continue

            result = run_single(name, target_ebops)
            if result:
                target_results.append(result)
                print(f'\n  ── Progress target={target_ebops} ({len(target_results)}/{len(exp_names)}) ──')
        ...
        if target_results and not args.dry_run:
            os.makedirs(output_root, exist_ok=True)
            with open(os.path.join(output_root, 'all_results.json'), 'w') as f:
                json.dump(target_results, f, indent=2, default=str)
            print_comparison(target_results, target_ebops)
            all_results_by_target[str(target_ebops)] = target_results

    if all_results_by_target and not args.dry_run:
        os.makedirs(OUTPUT_ROOT, exist_ok=True)
        with open(os.path.join(OUTPUT_ROOT, 'all_results_by_target.json'), 'w') as f:
            json.dump(all_results_by_target, f, indent=2, default=str)

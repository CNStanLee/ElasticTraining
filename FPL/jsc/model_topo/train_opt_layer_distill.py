...
def parse_ebops_from_name(path: Path) -> float:
    m = re.search(r"-ebops(\d+(?:\.\d+)?)", path.name)
    return float(m.group(1)) if m else float("inf")


def pick_best_target_meta(pruned_root: Path, target: int) -> Path:
    ...
    ranked = sorted(cands, key=lambda p: (abs(parse_ebops_from_name(p) - float(target)), -p.stat().st_mtime))
    return ranked[0]


class MeasuredEbopsCallback(keras.callbacks.Callback):
    def __init__(self, sample_input, log_every: int = 20):
        super().__init__()
...
class LayerDistillCallback(keras.callbacks.Callback):
    def __init__(
        self,
        teacher_model,
        sample_input,
        ...
        shift_clip: float = 2.0,
    ):
        super().__init__()
        self.teacher_model = teacher_model
        self.sample_input = sample_input
        self.get_q_var = get_q_var
    ...
    def on_epoch_end(self, epoch, logs=None):
        e = int(epoch) + 1
        if e < self.start_epoch or self.interval <= 0:
            return
        if (e - self.start_epoch) % self.interval != 0:
            return
        from utils.layer_distill_utils import teacher_guided_layer_distill

        delta = teacher_guided_layer_distill(
            student_model=self.model,
            teacher_model=self.teacher_model,
            sample_input=self.sample_input,
            get_q_var=self.get_q_var,
            passes=self.passes,
            scale_min=self.scale_min,
            scale_max=self.scale_max,
            shift_clip=self.shift_clip,
        )
        print(f"  [LayerDistill] epoch={e} passes={self.passes} mean|scale-1|={delta:.4f}")
        if logs is not None:
            logs["layer_distill_delta"] = float(delta)
...
def main() -> None:
    ...
    get_q_var = rbu._get_kq_var

    parser = argparse.ArgumentParser(description="Train opt-pruned model with high-EBOPs layer distillation")
    ...
    parser.add_argument(
        "--teacher_model",
        type=str,
        default="baseline/epoch=107802-val_acc=0.753-ebops=1599-val_loss=0.708.keras",
    )
    parser.add_argument("--pruned_root", type=str, default="pruned_models")
    parser.add_argument("--student_meta", type=str, default=None)
    ...
    parser.add_argument("--distill_sample_size", type=int, default=8192)
    parser.add_argument("--distill_pre_passes", type=int, default=6)
    parser.add_argument("--distill_interval", type=int, default=10)
    parser.add_argument("--distill_start_epoch", type=int, default=20)
    parser.add_argument("--distill_passes", type=int, default=1)
    parser.add_argument("--distill_scale_min", type=float, default=0.85)
    parser.add_argument("--distill_scale_max", type=float, default=1.15)
    parser.add_argument("--distill_shift_clip", type=float, default=2.0)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--out_dir", type=str, default="opt_train_layer_distill")
    ...
    teacher_path = here / args.teacher_model
    ...
    meta_path = (
        Path(args.student_meta).resolve()
        if args.student_meta
        else pick_best_target_meta(pruned_root=pruned_root, target=int(args.target))
    )
    ...
    student = keras.models.load_model(base_model, compile=False)
    ...
    n_distill = int(np.clip(args.distill_sample_size, 512, x_train.shape[0]))
    distill_input = tf.constant(x_train[:n_distill], dtype=tf.float32)

    pre_delta = 0.0
    if int(args.distill_pre_passes) > 0:
        pre_delta = teacher_guided_layer_distill(
            student_model=student,
            teacher_model=teacher,
            sample_input=distill_input,
            get_q_var=get_q_var,
            passes=int(args.distill_pre_passes),
            scale_min=float(args.distill_scale_min),
            scale_max=float(args.distill_scale_max),
            shift_clip=float(args.distill_shift_clip),
        )
    ...
    ebops_cb = FreeEBOPs()
    measured_ebops_cb = MeasuredEbopsCallback(sample_input=sample_input, log_every=int(args.log_every))
    set_all_beta(student, float(args.beta_init))
    ...
    distill_cb = LayerDistillCallback(
        teacher_model=teacher,
        sample_input=distill_input,
        get_q_var=get_q_var,
        interval=int(args.distill_interval),
        start_epoch=int(args.distill_start_epoch),
        passes=int(args.distill_passes),
        scale_min=float(args.distill_scale_min),
        scale_max=float(args.distill_scale_max),
        shift_clip=float(args.distill_shift_clip),
    )
    callbacks = [
        ...
        *([distill_cb] if int(args.distill_passes) > 0 and int(args.distill_interval) > 0 else []),
        measured_ebops_cb,
    ...
    print(f"[LOAD] student_meta={meta_path.name}")
    print(f"[LOAD] student_weights={weights_path.name}")
    print(f"[LOAD] teacher={teacher_path.name}")
    print(f"[DISTILL] pre_passes={args.distill_pre_passes} sample={n_distill} pre_delta={pre_delta:.4f}")
    print(f"[DISTILL] interval={args.distill_interval} start={args.distill_start_epoch} passes={args.distill_passes}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title(f"Layer Distill Train Curve (target={args.target})")
    ...
    final_ebops = compute_model_ebops(student, sample_input)

    summary_path = out_dir / "summary.txt"
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"student_meta={meta_path}\n")
        f.write(f"student_weights={weights_path}\n")
        ...
        f.write(f"distill_pre_delta={pre_delta:.6f}\n")
